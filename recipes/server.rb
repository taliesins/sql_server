...
is_iso = ['.iso', '.7z', '.zip'].include? fileextension
...
remote_file download_path do
  ...
  only_if { is_iso && !is_sqlserver_installed }
end
...
execute 'extract_iso' do
  ...
  only_if { is_iso && !is_sqlserver_installed && !(::File.directory?(iso_extraction_dir)) }
end
...
template config_file_path do
  ...
  only_if {!is_sqlserver_installed}
end
